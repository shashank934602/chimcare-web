...
function build(routes: RouteSummary[], geoRaw: unknown): MigratedState[] {
  ...
  for (const r of routes) {
    // Every URL shape, not just `{service}-in-{city}-{st}`: Seattle (`chimney-sweep-seattle-wa`), Portland
    // (`…-portland-oregon`) and Bedford (`bedford-chimney-sweep`, state from its title) used to be dropped
    // here, which left Washington, Oregon, Connecticut, New Hampshire and Rhode Island with no hub.
    const place = placeFromSlug(r.slug, r.title);
    if (!place || place.duplicate) continue; // no place at all, or a `-2`/`-3` duplicate (see the note above)
    const st = place.code.toLowerCase();
    parsed.push({ r, key: `${st}/${place.citySlug}`, st, code: place.code, citySlug: place.citySlug, cityName: place.city });
  }
